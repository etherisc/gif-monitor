...
Meteor.publish("product_list1", function() {
	return Products.find({}, {});
});

Meteor.publish("product1", function(productId) {
	return Products.find({_id:productId}, {});
});

Meteor.publish("product_list1_paged", function(extraOptions) {
	extraOptions.doSkip = true;
	return Products.find(databaseUtils.extendFilter({}, extraOptions), databaseUtils.extendOptions({}, extraOptions));
});

Meteor.publish("product_list1_paged_count", function(extraOptions) {
	Counts.publish(this, "product_list1_paged_count", Products.find(databaseUtils.extendFilter({}, extraOptions), { fields: { _id: 1 } }));
});

Meteor.methods({
	"productList1PagedExport": function(extraOptions, exportFields, fileType) {
